...
spec = do
  ...
  context "part B (only 50 houses)" $ do
    let lowestHouse = lowest presentsB

    context "using the problem input" $ do
      it "returns the lowest house for 33100000" $ do
        lowestHouse 33100000 `shouldBe` 786240

  context "part A (infinite houses)" $ do
    ...
    context "using the problem input" $ do
      let lowestHouse = lowest presentsA
      it "returns the lowest house for 33100000" $ do
